...
static jmethodID on_audio_level_changed_id;
...
static gboolean message_element_cb(GstBus *bus, GstMessage *msg, CustomData *data) {
    JNIEnv *env = get_jni_env();
    if (msg->type == GST_MESSAGE_ELEMENT) {
        ...
        if (strcmp(name, "level") == 0) {
            ...
            GST_DEBUG("Audio level changed : %f", rms);

            (*env)->CallVoidMethod(env, data->app, on_audio_level_changed_id, (jfloat) ((float)rms_dB));
            if ((*env)->ExceptionCheck(env)) {
                GST_ERROR("Failed to call Java method");
                (*env)->ExceptionClear(env);
            }
...
static void *app_function(void *userdata) {
    ...
    g_object_set(data->pipeline, "audio-sink", leveledsink, NULL);
...
static void gst_native_init(JNIEnv *env, jobject thiz) {
    ...
    GST_DEBUG_CATEGORY_INIT(debug_category, "gst-player", 0, "GstPlayer React Native");
    gst_debug_set_threshold_for_name("gst-player", GST_LEVEL_DEBUG);
...
static jboolean gst_native_class_init(JNIEnv *env, jclass klass) {
    ...
    on_media_size_changed_method_id = (*env)->GetMethodID(env, klass, "onMediaSizeChanged",
                                                          "(II)V");
    on_audio_level_changed_id = (*env)->GetMethodID(env, klass, "onAudioLevelChange", "(F)V");

    if (!custom_data_field_id || !set_message_method_id || !on_gstreamer_initialized_method_id ||
        !on_media_size_changed_method_id || !set_current_position_method_id ||
        !on_audio_level_changed_id) {
        /* We emit this message through the Android log instead of the GStreamer log because the later
         * has not been initialized yet.
         */
        __android_log_print(ANDROID_LOG_ERROR, "gst-player",
